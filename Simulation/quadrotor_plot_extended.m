...
function sys = mdlOutputs(t,u,s, plot, enable, quad)
    ...
    if enable == 1
        ...
        load = F*load;
        if (tension < quad.m*quad.g)
            plot3([hub0(1) load(1)], [hub0(2) load(2)], [hub0(3) load(3)],'-k');
        elseif (tension == quad.m*quad.g)
            plot3([hub0(1) load(1)], [hub0(2) load(2)], [hub0(3) load(3)],'-g');
        elseif (tension > quad.m*quad.g)
            plot3([hub0(1) load(1)], [hub0(2) load(2)], [hub0(3) load(3)],'-r');
        end
        
        plot3([load(1)],[load(2)],[load(3)],'o');
